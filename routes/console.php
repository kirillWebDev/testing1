...
Artisan::command('avatar', function () {
    $this->comment(\App\Models\User::whereNull('avatar')->update(['avatar' => config('app.avatar')]));
})->describe("set users's null avatar to default avatar");


Artisan::command('xssProtection', function () {
    $mp = new MarkDownParser();
    foreach (\App\Models\Comment::withoutGlobalScopes()->get() as $comment) {
        $this->comment("----------------------------------------------------------------------------------------\n");
...
Artisan::command('comment:delete-uv', function () {
    $result = \App\Models\Comment::withoutGlobalScope(VerifiedCommentScope::class)->where('status', 0)->delete();
    $this->comment("Delete $result comments.");
    cache()->flush();
})->describe("delete un verified comments");

Artisan::command('ip:delete-ub', function () {
    $result = \App\Models\Ip::where('blocked', 0)->delete();
